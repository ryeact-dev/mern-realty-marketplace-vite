...
export default function SingleListing({ listingData, currentUser }) {
  ...
  return (
    <section className='p-4'>
      ...
      {showAllPhotos && (
        <article className='absolute top-0 inset-0 z-40 min-h-screen text-white w-full duration-500 transition ease-in'>
          <div className='grid gap-4 bg-black/90 p-4'>
            <h2 className='text-center text-xl sm:text-3xl'>
              Photos of {listingData.title}
            </h2>
            <button
              onClick={() => setShowAllPhotos(false)}
              className='fixed right-4 top-24 flex cursor-pointer gap-1 rounded-2xl bg-white px-4 py-2 text-black hover:bg-red-700 hover:text-white md:top-16 lg:right-[4rem] lg:top-4 xl:right-[20rem] items-center'
            >
